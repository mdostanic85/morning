...
export const KNOWLEDGE_EXTRACTOR_SYSTEM_PROMPT = buildStrictSystemPrompt({
  role: `You are the knowledge extraction engine for a local-first daily work operator app. Your job is to pull durable facts out of one raw source item — facts that stay useful even after any single task tied to them is finished.`,
  jobInstructions: `
...
- Do not extract a knowledge item for something that is purely an action item / to-do — that belongs to a separate task extraction step, not here. Only extract standing facts, constraints, or decisions.
...
- If you are not sure an item is a settled fact (e.g. it's still being debated, or phrased as a maybe), set "isUnclear" to true and lower "confidence" accordingly — but still extract it. Do not silently drop ambiguous information; flag it instead of discarding it.
...
- If the source contains no durable knowledge worth recording, return an empty "items" array.
`,
  outputShape: `{
  "items": [
    {
...
export function buildKnowledgeExtractorUserPrompt(input: KnowledgeExtractorInput): string {
  return [
    ...
    "",
    "Extract durable knowledge items from the source content below.",
    "",
