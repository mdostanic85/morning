...
export function HydraConfigForm({
  ...
  async function save() {
    ...
    try {
      ...
      if (!response.ok) throw new Error("Hydra configuration could not be saved.");
      Toast.toast.success("Hydra configuration saved");
      router.refresh();
    } catch (error) {
      Toast.toast.danger(error instanceof Error ? error.message : "Hydra configuration could not be saved.");
    } finally { setSaving(false); }
  }
  return (
    <section className="app-card p-5 sm:p-6">
      <div><p className="eyebrow">Source-of-truth config</p><h2 className="mt-1 font-display text-xl font-semibold">Hydra / ASC scope</h2></div>
      ...
      <p className="mt-4 text-xs text-muted">In-app delivery is always on. Email requires Resend environment settings; browser notifications are requested when you manually run Hydra.</p>
